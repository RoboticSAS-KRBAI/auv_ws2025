...
from std_msgs.msg import String, Float32
from auv_interfaces.msg import SetPoint, MultiPID, PID, ObjectDetection
...
class Guidance(Node):
    ...
    def __init__(self):
        ...
        self.search_start_time = self.now()
        self.scan_toggle_time = self.now()
        self.gate_last_seen = self.now()
        ...
        pid_pitch = PID()
        pid_pitch.kp = 9.0 #15.0
        pid_pitch.ki = 0.0
        pid_pitch.kd = 1.7 #2.6
    ...
    def start_auv(self):
        ...
        if self.state == "STABILIZE":
            ...
            if self.elapsed() > 3.0:
                self.change_state("SEARCH_FLARE")

        # =============================
        # SEARCH FLARE (SCAN YAW)
        # =============================
        elif self.state == "SEARCH_FLARE":

            # jika flare ditemukan langsung dodge
            if self.object_class == "orange_flare" or self.object_class == "red_flare" or self.object_class == "yellow_flare":
                self.get_logger().info("ORANGE FLARE DETECTED")
                self.change_state("DODGE_FLARE")
                return
            
            search_elapsed = self.now() - self.search_start_time
            scan_elapsed = self.now() - self.scan_toggle_time
            ...
            # kalau 5 detik tidak ada flare → maju
            if search_elapsed > 2.0:
                self.set_point.yaw = self.base_yaw
                self.pub_set_point.publish(self.set_point)

                self.get_logger().info("MAJU DULU")
        ...
        # =============================
            ...
            status_msg.data = "camera_sway_forward_right"
            self.pub_status.publish(status_msg)
            ...
            if self.object_class == "orange_flare":
                ...
                if self.flare_orange_detected_once:
                    ...
                    if lost_time > 2:
                        self.get_logger().info("FLARE DODGED woi")
